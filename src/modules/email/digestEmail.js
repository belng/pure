...
async function sendDigestEmail () {
	...
	pg.readStream(connStr, {
		$: `SELECT
					...
					FROM roomrels, threads, rooms, users
						WHERE
							...
							AND roles @> '{3}'
				ORDER BY users.id, scoreperfollower DESC limit 10`,
		range
	}).on('row', async row => {
		// console.log(row);
		if (row.userid !== puser.id && threadRels.length > 0) {
			...
			// await initMailSending({threadRels, user: puser});
			threadRels = [];
		}
		puser.id = row.userid;
		...
		i++;
		log.info('Sending meantion email to last user: ', puser, threadRels.length);
		log.info('Got ', i++, ' users');
		// initMailSending({threadRels, user: threadRels});
