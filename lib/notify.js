...
function dispatch(changes, core, options) {
	...
	for (let key in changes.entities) {
		console.log(key);
		if (!groups[changes.entities[key].parent]) {
			groups[changes.entities[key].parent] = {};
		}
		groups[changes.entities[key].parent][changes.entities[key].id] = changes.entities[key];
	}
